...
def load_users():
    if os.path.exists('users.csv'):
        with open('users.csv', mode='r', newline='') as file:
            ...
            for row in reader:
                if len(row) >= 4:  # Expecting username, password, email, selected_dates
                    username, password, email, *dates = row
                    users[username] = {
                        'password': password,
                        'email': email,
...
def save_users():
    with open('users.csv', mode='w', newline='') as file:
        ...
        for username, user_data in users.items():
            row = [username, user_data['password'], user_data['email']] + user_data['selected_dates']
            writer.writerow(row)
...
def sign_up():
    ...
    sign_up_button = tk.Button(sign_up_window, text="Sign Up", command=lambda: sign_up_button_click(username_entry, password_entry, email_entry))
    sign_up_button.pack()
...
# Function to handle right-click to remove date
